    ...
    class Solution {
        public int myAtoi(String str) {
           ...
           if (chars[index] == '-') {
               ...
               index ++;
           }else if (!Character.isDigit(chars[index])) return 0;
           int result = 0;
           while (index < len && Character.isDigit(chars[index])) {
               int digit = chars[index] - '0';
               if (result > (Integer.MAX_VALUE-digit) / 10) {
                   return supOrplus?Integer.MAX_VALUE : Integer.MIN_VALUE;
               }
               result = result*10 +digit;
               index++;
           }
           return supOrplus?result : -result;
        }
    }
